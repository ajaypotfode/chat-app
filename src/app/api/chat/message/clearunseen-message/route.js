...
export const POST = async (req) => {
    ...
    try {
        ...
        await Messages.findOneAndUpdate(
            { chatId },
            { $set: { 'messages.$[msg].seen': true } },//this is use to target element from an array "messages.$[element].seen"(msg will be anything eg.msg)
            {
                arrayFilters: [
                    { 'msg.sender': { $ne: userId }, 'msg.seen': false }
                ]
                //it is use to filter the object from array(msg will be anything)
            }
        );
